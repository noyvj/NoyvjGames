...
| continuum | Continuum (working title) | Milestone 13 done (Space Age era) — Phase 1 (Foundation), Phase 2 (story/era-transition framework), Phase 3 (content expansion), Phase 4 (save/security audit) and Phase 5 (visual & accessibility) all complete: all seven eras (Tribal through Space Age) fully built, each with sim content, a research tier, a sustainability mechanic, log/transition beats, info-panel content, and now a real, live-verified low-poly Three.js scene — Industrial, Digital and Space Age each add a distinct mechanical consequence rather than a repeated score-side bonus/penalty shape (Industrial/Digital growth-side, Space Age a new fourth livability provision instead of a third growth lever). Phase 4's dedicated save/security audit found and fixed six real save-robustness bugs (an era-restoration crash reachable via revisit, adversarial numeric values, a NaN/Infinity gap, a growth-loop page-freeze, revisit era cross-contamination, and a log-restore crash). Phase 5 added Three.js (pinned r128, the hub's first third-party JS dependency and first WebGL) reading a new `visual.py` state-contract module, with a real scene for all seven eras verified live through every era transition, a WebGL-unavailable 2D fallback (the pre-existing space-theme diorama), a real text-size accessibility control, and a colorblind-safety audit (Okabe-Ito palette) fixing one hue-only good/bad log color pair. Test count 372 → 396. Phase 6 (hub integration): linked into the main hub nav (title card + review widget, same pattern as every other game) — hub-nav portion done; full-playthrough save/revisit integration testing and max test-coverage pass still open. 19 achievements + in-game panel + unlock toast + `achievements_earned` in its save state, plus a new "Look Back" era-revisit UI (closing a gap where the save system's revisit feature had no way to trigger it). Flagship BCM114 Round 2 DA (SDG 11). Hub-linked |
