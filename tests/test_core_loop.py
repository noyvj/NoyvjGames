"""Tests for the Milestone 1 core click loop (game.py)."""
...
def test_multiple_clicks_accumulate(game_env):
    for _ in range(5):
        game_env.click()
    assert game_env.module.resource_count == 5
    assert game_env.elements["resource-count"].innerText == "5"


def test_click_calling_on_click_directly_also_increments(game_env):
    game_env.module.on_click(None)
    assert game_env.module.resource_count == 1
...
def test_rapid_clicks_each_schedule_their_own_clear(game_env):
    ...
    game_env.timers.flush()
    assert not button.classList.contains("pressed")
    assert game_env.module.resource_count == 3
...
def test_update_display_reflects_arbitrary_module_state(game_env):
    game_env.module.resource_count = 42
    game_env.module.update_display()
