...
def test_load_button_disabled_when_already_loaded(game_env):
    game_env.load()
    assert game_env.elements["load-button"].disabled is True


def test_depart_button_disabled_until_loaded(game_env):
    assert game_env.elements["depart-button"].disabled is True
    game_env.load()
    assert game_env.elements["depart-button"].disabled is False
...
def test_depart_starts_transit_to_other_colony(game_env):
    game_env.load()
    game_env.depart()
...
def test_render_reflects_transit_status_text(game_env):
    game_env.load()
    game_env.depart()
    assert "In transit" in game_env.elements["ship-status"].innerText
    game_env.tick(game_env.module.TRAVEL_TICKS)
    assert "Docked at Verdant Reach" in game_env.elements["ship-status"].innerText
