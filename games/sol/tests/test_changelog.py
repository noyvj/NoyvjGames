...
planning/TODO.md, origin K16): SOL's own changelog.json catalog and the
...
def test_tick_keeps_the_open_panel_live(game_env):
    """Pins that update_changelog_display() is wired into tick() (via
    _full_render()'s per-tick refresh block), not only into on_toggle_changelog()
    -- mirrors test_achievements.py's own tick-liveness test for its panel."""
    module = game_env.module
    module.on_toggle_changelog()
