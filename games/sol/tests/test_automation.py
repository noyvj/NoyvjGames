"""Tests for the Milestone 2 auto-generator (passive Iron production) on Earth."""
...
def test_generator_cost_increases_after_purchase(game_env):
    first_cost = game_env.module.generator_cost("Earth")
    game_env.earth["resource_count"] = first_cost
    game_env.buy_generator()
    second_cost = game_env.module.generator_cost("Earth")
...
def test_buy_button_label_reflects_next_cost(game_env):
    ...
    game_env.buy_generator()
    button = game_env.elements["buy-generator-button"]
    expected_cost = game_env.module.generator_cost("Earth")
...
def test_owning_a_generator_produces_resources_over_ticks(game_env):
    ...
    assert math.isclose(game_env.earth["resource_count"], 1.0, abs_tol=1e-9)
...
def test_multiple_generators_scale_the_rate_linearly(game_env):
    ...
    assert math.isclose(game_env.earth["resource_count"], 3.0, abs_tol=1e-9)
...
def test_manual_clicking_still_works_alongside_automation(game_env):
    ...
    assert math.isclose(game_env.earth["resource_count"], 2.0, abs_tol=1e-9)
