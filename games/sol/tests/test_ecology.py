"""Tests for the Milestone 3 ecology system: sustainability meter, the
penalty/halt curve, and Recycler buildings that recover ecological health."""
...
def test_multiplier_is_zero_below_zero(game_env):
    ...
    game_env.module.ecology_health = -5.0
    assert game_env.module.production_multiplier() == 0.0
...
def test_generator_decays_ecology_over_ticks(game_env):
    ...
    assert math.isclose(game_env.module.ecology_health, 99.0, abs_tol=1e-6)
...
def test_multiple_generators_decay_ecology_faster(game_env):
    ...
    assert math.isclose(game_env.module.ecology_health, 97.0, abs_tol=1e-6)
...
def test_ecology_health_clamps_at_zero_and_does_not_go_negative(game_env):
    ...
    assert game_env.module.ecology_health == 0.0
...
def test_generator_output_penalized_in_low_ecology_band(game_env):
    ...
    game_env.module.ecology_health = 5.0  # in the 0.75x penalty band
    game_env.module.resource_count = 0
    game_env.module.tick()
    # 1 generator * 1 iron/s * 0.1s * 0.75 penalty
    assert math.isclose(game_env.module.resource_count, 0.075, abs_tol=1e-9)
...
def test_generator_output_full_rate_above_threshold(game_env):
    game_env.module.generator_count = 1
    game_env.module.ecology_health = 100.0
    game_env.module.resource_count = 0
    game_env.module.tick()
    assert math.isclose(game_env.module.resource_count, 0.1, abs_tol=1e-9)
...
def test_manual_click_is_never_penalized_by_low_ecology(game_env):
    ...
    # permanently stuck (see comment above on_click in game.py).
    game_env.module.ecology_health = 5.0
    game_env.click()
...
def test_status_message_at_critical_band(game_env):
    game_env.module.ecology_health = 5.0
    game_env.module.update_ecology_display()
...
def test_buying_recycler_deducts_cost_and_increments_count(game_env):
    game_env.module.resource_count = 15
    game_env.buy_recycler()
...
def test_recycler_cost_increases_after_purchase(game_env):
    first_cost = game_env.module.recycler_cost()
    game_env.module.resource_count = first_cost
    game_env.buy_recycler()
    second_cost = game_env.module.recycler_cost()
...
def test_recycler_button_label_reflects_next_cost(game_env):
    ...
    game_env.buy_recycler()
    button = game_env.elements["buy-recycler-button"]
    expected_cost = game_env.module.recycler_cost()
...
def test_recycler_restores_ecology_over_ticks(game_env):
    ...
    game_env.timers.tick_intervals(10)  # 1 second
    assert math.isclose(game_env.module.ecology_health, 52.0, abs_tol=1e-6)
...
def test_recyclers_and_generators_net_out_correctly(game_env):
    ...
    game_env.buy_generator()
    game_env.buy_recycler()
    game_env.module.ecology_health = 80.0
    game_env.timers.tick_intervals(50)  # 5 seconds
    assert math.isclose(game_env.module.ecology_health, 80.0, abs_tol=1e-6)
...
def test_recycler_alone_can_recover_from_total_collapse(game_env):
    ...
    assert math.isclose(game_env.module.ecology_health, 2.0, abs_tol=1e-6)
...
def test_full_collapse_and_recovery_is_never_a_dead_end(game_env):
    ...
    game_env.module.ecology_health = 0.0
    game_env.module.resource_count = 0
    game_env.module.generator_count = 1  # would otherwise produce 0 (halted)
    ...
    for _ in range(15):
        game_env.click()
    assert game_env.module.resource_count == 15
...
def test_being_outpaced_by_generators_is_still_not_a_dead_end(game_env):
    ...
    game_env.module.ecology_health = 0.0
    game_env.module.resource_count = 15
    game_env.module.generator_count = 5
    game_env.buy_recycler()  # 1 recycler: restore 2%/s vs decay 5%/s

    game_env.timers.tick_intervals(10)  # 1 second
    assert game_env.module.ecology_health == 0.0  # net negative, still stuck

    # Manual clicking remains available regardless, funding more Recyclers...
    for _ in range(200):
        game_env.click()
    while game_env.module.resource_count >= game_env.module.recycler_cost():
        game_env.buy_recycler()
    assert game_env.module.recycler_count >= 3  # enough to outpace 5%/s decay
