...
planning/TODO.md, origin K16): Tide's own changelog.json catalog and the
...
def test_entries_are_sorted_newest_first(game_env):
    """changelog.json is hand-authored newest-first, but CHANGELOG sorts
    defensively at load time in case a future entry is appended out of
    order -- unlike Grid/SOL, which trust the JSON's own order."""
    module = game_env.module
...
def test_render_keeps_the_open_panel_live(game_env):
    ...
    on every action), not only into on_toggle_changelog() -- mirrors
...
def test_toggling_does_not_mutate_settlement_state(game_env):
    """The changelog is a read-only informational panel -- opening it
    should never touch the game's own SettlementState."""
    module = game_env.module
