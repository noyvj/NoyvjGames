...
class FakeElement:
    def __init__(self, id_=None, registry=None):
        ...
        self.title = ""
        self.className = ""
        self.value = ""  # G4 -- the strategy-label text inputs
...
class FakeProxy:
    ...
    that manages a one-shot proxy's lifetime (e.g. the achievement
    toast's setTimeout callback) can be exercised under test the same way
    it behaves against real Pyodide."""
...
class FakeTimers:
    ...
    so tests can assert on pre/post-flush state (used by the
    achievement-unlock toast's auto-hide timer). Same shape as
    Grid/Continuum's own tests/fakes.py FakeTimers."""
