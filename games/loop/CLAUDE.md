...
## Material passport (H21, 2026-09-26)

A literal traced object: one unit of the chain's material ('Unit #1') and its journey. Each cycle `ChainState.passport_source()` decides where the unit comes from (newly mined, kept in use by repair, passed on by reuse, recovered by recycling, or brought in by trade), drawn from the real supply mix by a fixed golden-ratio sequence, so there is no randomness and a run is reproducible and consistent with the numbers (a straight-line chain is always 'newly mined', a fully closed chain never is). The history (`passport`, capped at `PASSPORT_MAX_ENTRIES` 40) is saved only when non-empty, entries validated on load (cycle int >= 1, known source), and cleared by Start New Chain. A collapsed 'Material passport' panel shows a tally (newly mined N, kept in the loop M, longest unbroken run) and the latest eight cycles newest first. 263 -> 276 tests (`tests/test_passport.py`).

## Product lifecycle story (H7, 2026-09-26)

Builds on the H21 material passport: the traced unit is a NAMED product per goods category (`PASSPORT_PRODUCT_NAME`: Nova the phone, Juniper the jacket, Oak the chair) and every journey step gets a sentence written for that category and source (`PASSPORT_STORY`, 5 sources by 3 categories, so a recycled chair is chipped into new board while a recycled phone is stripped for its metals). `passport_story_lines()` numbers the product's lives: each step that keeps it in the loop is its next life and a fresh mining resets the count, which gives the thread a shape. The passport panel now shows these sentences (latest eight, newest first) with the tally headed by the product's name; it is derived from the saved passport, so nothing new is saved. Unknown categories fall back to the default. 276 -> 283 tests (`tests/test_passport_story.py`); verified live.
