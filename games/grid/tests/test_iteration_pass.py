...
def test_trend_graph_svg_empty_with_fewer_than_two_points(game_env):
    ...
    assert game_env.module.trend_graph_svg([5.0], [10.0], [8.0]) == ""


def test_trend_graph_svg_contains_three_polylines(game_env):
...
def test_trend_graph_svg_point_count_matches_history_length(game_env):
    svg = game_env.module.trend_graph_svg(
        [0.0, 5.0, 10.0, 15.0], [150.0, 140.0, 130.0, 120.0], [0.0, 8.0, 16.0, 24.0]
    )
...
def test_render_shows_trend_graph_after_two_rounds(game_env):
    ...
    assert "emissions" in game_env.elements["trend-graph-message"].innerText.lower()
