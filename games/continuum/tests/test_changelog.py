...
def test_every_entry_has_a_real_date_and_nonempty_text(game_env):
    ...
    for entry in module.CHANGELOG:
        ...
        # Raises ValueError if not a real ISO date -- catches typos like
        # "2026-13-01" or a stray non-date string slipping into the JSON.
...
def test_closing_the_panel_does_not_clear_it_the_next_open(game_env):
    # Regression guard: update_changelog_display() clears panel.innerHTML
    # only when opening, never poisons state on close.
    game_env.toggle_changelog()
    game_env.toggle_changelog()
    game_env.toggle_changelog()
    panel = game_env.elements["changelog-panel"]
