...
- **Password-reset path for accounts** (found during the planning-doc audit, not from your answers) — `planning/ACCOUNTS-AND-FEEDBACK-DESIGN.md` flags that username+password auth with no email on file means a forgotten password permanently strands that account's saves. Needs a real decision: add an email provider (the exact cost/complexity that was dropped when magic-link auth got replaced), or a lower-tech option like an admin-assisted manual reset — reasonable at this site's current size. Flagging rather than picking one for you since it's a real tradeoff.
