# Site-Wide TODO — Round 2

**Progress: 1/708 items checked off (0%).** Recompute with `grep -c "^\s*- \[[ x]\]" planning/TODO2.md` (total) and the same with `\[x\]` (done) as items land. The count is high because every cross-game rollout item (Z section) is broken into one checkbox per game — see the note at the top of that section. `TODO2.md` is the working title on purpose — the user will rename it once the original `planning/TODO.md` is fully finished, so the two never collide.
...
- [ ] X-b **[BIG]**: Add a `lastData.dat` file-upload/import feature on top of X-a's architecture. There's no official Digital Extremes account-inventory API and AlecaFrame's own sync happens via Overwolf (a native desktop app), neither of which fits a website — but the open-source **[`Sainan/warframe-api-helper`](https://github.com/Sainan/warframe-api-helper)** tool (free, MIT-licensed, no password needed) produces a `lastData.dat` snapshot file that AlecaFrame also writes, and sites like `browse.wf/inventory` already prove parsing it 100% client-side (nothing sent to any server) is a real, working pattern. Needs a sample file or the helper's own source inspected first to nail down the exact format. Explicitly NOT building a native Overwolf-style companion app for continuous real-time sync — that's a different, much bigger project.
