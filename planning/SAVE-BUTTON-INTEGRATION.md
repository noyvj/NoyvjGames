...
- **On Save:** calls `window.pyodide.globals.get('get_state')()`, converts the Pyodide proxy to a plain JS object (`.toJs({dict_converter: Object.fromEntries})`), and `POST`s it to `/saves` (or `PUT`s to `/saves/{code}` if a code already exists this session). Shows the returned save code to the player, with a "Copy code" button next to it (falls back from the Clipboard API to a legacy `execCommand` copy if the former is unavailable/denied; shows a plain "couldn't copy" message rather than failing silently if both are blocked).
- **Remembers the code locally:** stores the last save code for *this game* in `localStorage` (keyed per game, e.g. `savecode:sol`), so returning to the same browser can offer "Continue" without retyping a code — the same remembered code also pre-fills the "Load by code" input (only when that field is empty, so it never clobbers a code the player is actively typing in).
