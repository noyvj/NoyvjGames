...
- **On Save:** calls `window.pyodide.globals.get('get_state')()`, converts the Pyodide proxy to a plain JS object (`.toJs({dict_converter: Object.fromEntries})`), and `POST`s it to `/saves` (or `PUT`s to `/saves/{code}` if a code already exists this session). Shows the returned save code to the player.
  <!-- The widget doesn't copy the code to the clipboard or auto-fill the load
       input from the remembered code yet -- both were once described here as
       if built, which they weren't (caught 2026-09-14). Real, small features
       worth having, not just doc corrections -- tracked in planning/TODO.md's
       shared save-widget QoL section rather than described here as done. -->
