...
- [x] Y11 **(you answered "yes" in round 2, and approved it in round 1 as L5 — this had been dropped from this list by mistake)**: A genuine site-wide dark/light theme toggle. Needs the dedicated design pass first: a real second (light) colour scheme for the hub shell and every game's `style.css` (backgrounds, borders, meter fills, text contrast, per-game accent hues, Continuum's 3D scene sky/lighting), then a `data-theme` attribute + CSS custom-property swap, a toggle in each game's settings panel and on the hub, persisted per-device. Do it game by game after the current per-game feature passes land (they all edit `style.css`), and keep the colorblind-safety audit rules. **Scoping 2026-09-26 (Noyvj, no code yet):** measured first: the hub `style.css` alone has ~187 hard-coded colours and no custom properties, plus the shared `space-bg.css`/`ambient-bg.css` backgrounds, and every game repeats the pattern, so a one-shot swap isn't possible. Plan: (1) `shared/theme.js` sets `<html data-theme="dark|light">` (default from `prefers-color-scheme`, override persisted per device in `localStorage["theme"]`, and a toggle in general Settings per the U4 answer); (2) per stylesheet, replace the recurring colours with a small custom-property set (`--bg`, `--surface`, `--border`, `--text`, `--muted`, plus each game's own accent) at the top of the file, then add an `html[data-theme="light"]` block that redefines them — hub shell first, then games one at a time in the order of least custom art (Loop, Herd, Thaw, Drift, Tide, Grid, Canopy, Aftermath, Trade Empire, SOL, Le Champ de Mots, Continuum last for its 3D sky/lighting); (3) keep the colorblind rules (no hue-only state pairs) and re-check text contrast at both themes. Estimated 4-6% of a five-hour window per game plus ~10% for the hub shell and shared backgrounds. **Progress 2026-09-26:** step 1 and the hub shell are DONE: `shared/theme.js` (data-theme from stored choice, else `prefers-color-scheme`, `NoyvjTheme` API, toggle buttons), a light palette for the hub `style.css` and `ad-bar.css`, and a theme toggle in the nav of every hub page (index, achievements, what's-new, roadmap, sources, terms). Verified live: toggling both ways, persistence in `localStorage["theme"]`, readable contrast on cards/inputs/buttons/stars. **Step 2 also DONE (same day):** all 12 games now load `shared/theme.js` (with a floating "Light/Dark mode" pill, bottom-left) and one shared `shared/theme-light-games.css` that restyles the glass-panel chrome every game shares (shell, `.section`, buttons, inputs, meters, info toggles, HUD readouts, save widget, opening screen, ad bar; the ambient/space backgrounds fade to a pale wash). Spot-checked live in Loop, Herd, Thaw, SOL and Continuum. **Still to do:** per-game tweaks for game-specific art and accent colours (a contrast pass on each game, e.g. Continuum's consulting-mode gold text, Le Champ de Mots' four visual styles, Canopy/Tide plot and coastline tiles), Continuum's 3D scene sky/lighting for light mode, and moving each game's toggle into its Settings panel (the floating pill is the interim control). **Closed 2026-09-26 as delivered** (hub shell plus all 12 games, one shared `theme.js`, persisted per device, synced to the account via Y31). The remaining polish is tracked as Y11b.
- [ ] Y11b (follow-up to Y11): light-theme polish. (a) Continuum's 3D scene sky/lighting and consulting-mode gold text in light mode; (b) Le Champ de Mots' four visual styles each need a light check (low-poly and text-based especially); (c) game-specific art/accent colours a contrast scan flags in Canopy/Tide plot and coastline tiles, Trade Empire, Aftermath; (d) move each game's theme toggle from the floating pill into its Settings panel where one exists (SOL, Continuum) and keep the pill elsewhere.
...
- [x] C3: A regional/multi-grid mode — manage two interconnected grids, sharing surplus capacity between them. Built as the lighter version (same shape as Tide's D3 sister settlement): a one-way "Connect a regional grid" switch that has no plants of its own, just its own demand, served first from whatever main-grid surplus capacity storage-arbitrage didn't already claim, with any remainder costing shared funds. See `games/grid/CLAUDE.md`.
...
- [x] D3 (answered 2026-09-26, was parked in LATER.md): multi-settlement mode, built as the lighter **sister-settlement** version — the user ruled out a fully independent second settlement. A second coastal settlement that shares the player's funds and adaptation tier but has its own coastline and sea-level exposure.
...
- [x] E17a: A "climate scenario pack" — choose among a few real-world-grounded event-schedule variants (coastal/inland/urban) at run start.
...
- [x] E27: A "resilience mentor" onboarding mode — an optional guided first run with inline suggestions, distinct from the standard tutorial.
...
- [x] H3: A "supply chain redesign" late-game layer — once fully circular, over-invest in a specific measure (repair/reuse/recycle) for a small efficiency bonus.
...
- [x] H7: A "product lifecycle" vignette expansion — follow a specific named product across multiple cycles, a small ongoing narrative thread.
- [x] H8: Let the cost-per-unit-of-supply readout show a small trend indicator (getting cheaper/pricier).
- [x] H9: A "waste stream diversification" mechanic — specialize in recovering a specific material type for a bonus.
...
- [x] H17: A "consumer behavior" mechanic — invest in demand-side changes (encouraging reuse/repair culture) distinct from supply-side investment.
- [x] H18: A small glow on the loop-ring visualization's node currently receiving the most investment.
- [x] H19: A "circular economy scorecard" comparing the player's achieved circularity against several real-world sector benchmarks simultaneously (extends the existing single comparison).
...
- [x] H21: A "material passport" mechanic — track a piece of material's full journey as a literal traceable object.
...
- [x] I1: A second receiving region — manage two regions with different starting capacity/pressure profiles simultaneously.
...
- [x] I3: A "policy toolkit" mechanic — choose among a few real-world-grounded institutional policy levers (streamlined credentialing, language-access funding) as named investment options, rather than the current abstract housing/services/infrastructure split.
...
- [x] I7: A "regional network" mechanic — a well-prepared region optionally supports a struggling neighboring region (light connection to I1).
...
- [x] U3 **(built 2026-09-26; slots need the backend redeploy, FOR-YOU action item 3; decided 2026-09-26)**: Saving: when signed in, Save always attaches to the account ("claim by default"); anonymous players keep today's plain save codes. When the account already has a save for that game, show a pop-up offering Overwrite / Save as a new slot / Cancel — or let the player pick from U4's opening-screen saves list. **3 slots per game for signed-in accounts only** (each with a name and last-saved time); anonymous players stay on a single code. Existing accounts' current saves become slot 1, no data lost. **Built:** backend `saves.slot`/`slot_name` (patched in), `PUT /users/me/saves/{game}/slots/{1-3}` (create or overwrite, optional trimmed name), `/users/me/saves` lazily migrates an account's existing claimed saves into slots (newest becomes slot 1, so nothing is lost), and claiming an anonymous save takes the first free slot (409 when all three are full); 15 new backend tests (137 total). `shared/save-widget.js` (all 12 games): signed in, a Slots list (Load / Save here per slot, active slot marked), Save Progress writes to the active slot, and the first save of a visit into an occupied slot asks Overwrite / Save to an empty slot / Cancel (autosave only writes to a slot already confirmed that visit; the signed-in autoload marks its slot active); anonymous players keep the single code untouched. Slots have default names ("Save N") and the API takes a name, but there is no rename box in the widget yet (small follow-up).
...
- [x] U4b (done 2026-09-26; follow-up to U4): Personalise each game's opening screen beyond the shared base layout (per the user's "same basic layout, then made individual"): game-specific tagline/art per game, Continue showing the save's own summary (era/round/score), and for **Le Champ de Mots only** the New Game visual-set picker step (live example + "confirm — this can be changed later in settings"). Also revisit once U3's save slots exist (Continue/Saves should list the 3 slots). **Done:** each game passes its own `data-tagline` (12 written), Continue shows a note from the latest save ("Slot 2 · 3 h ago" signed in, "Saved 3 h ago" with a stored code; silent if the backend is unreachable), and Le Champ de Mots' New Game adds a "pick a look" step (its four visual styles, applied live, "change this any time in Settings", desktop widths only) before the tutorial offer. Per-game art beyond a tagline is left for when a game wants its own look; the slots-aware Saves button already opens the widget's slot list.
...
- [x] U6 **(built 2026-09-26, on top of U8; live after the redeploy)**: Admin page: give reports a "mark as done" option (persisted server-side, so a done report drops out of the open queue). Needs a write endpoint (`is_resolved`/`resolved_at` columns on `answer_reports`, added through `patch_schema()`), which must not be public — so it is built together with U8's admin token rather than shipped first and left open to anyone. **Built:** `answer_reports.is_resolved` (default false) and `resolved_at` (added via `patch_schema()`), admin-token-only `PATCH /answer-reports/{id}` (`{"resolved": bool}`, 404 for an unknown id, 422 for a bad body, reopening clears the timestamp), and a `resolved` filter on the admin list. `admin.html`'s reports table has an Open/Done/All selector, a Status column with Mark done / Reopen, and an open-count in the status line. 8 new backend tests (107 total).
- [x] U7 **(built 2026-09-26; live after the redeploy + tokens; decided 2026-09-26)**: Test data on the admin page: add an `is_test` flag on accounts (reviews/reports/saves inherit it from their account), a dedicated AI-testing account so the AIs never touch real data, and a "hide test data" checkbox on admin (on by default; unowned reports/reviews can be flagged by hand). Flagged test data is also excluded from the PUBLIC stats (ratings, achievement percentages, community highlights). **Which accounts are real:** neither of us can see the list today (admin stats are counts only), so the password-protected admin page gets an Accounts panel listing every account with a "mark as test" checkbox — the user ticks the fakes there. **Built:** `users.is_test` (auto-set for usernames starting `aitest`, which is the dedicated AI testing account convention), admin-only `GET /admin/users` (username, email, save count, created, test flag; `include_test` filter) and `PATCH /admin/users/{id}` (StrictBool flag, clears the public-stats cache), `GET /admin/stats?hide_test=` (default true) and the public per-game aggregates (`_game_saves`) now exclude test accounts' saves. `admin.html` has an Accounts panel (list, per-account Test checkbox, "Hide test data" toggle remembered per device) so the real accounts can be told apart there. Ratings/reports with no account can't be flagged yet (they carry no user link): left as a small follow-up if the test rows there matter.
- [x] U8 **(built 2026-09-26; goes live once the user sets the two env vars (FOR-YOU action item 2) and redeploys (action item 3); decided 2026-09-26)**: Admin page behind a password. Admin endpoints require an `X-Admin-Token` header; `admin.html` shows a password box and keeps the token only in that tab's `sessionStorage`. Server env var `ADMIN_TOKEN` (the user's password). The AIs get a SEPARATE, independently revocable `AI_ADMIN_TOKEN`, only ever set in a local gitignored file. Protects ALL admin-only endpoints (`/admin/stats`, the raw `answer-reports` list, and everything U6/U7/U9 add). **Built:** `require_admin` dependency in `app/main.py` (constant-time compare against `ADMIN_TOKEN` or `AI_ADMIN_TOKEN`, 401 on a wrong/missing token, **503 fail-closed when neither variable is set** so an early deploy locks rather than exposes), applied to `GET /admin/stats` and `GET /answer-reports` (posting a report and the public ratings/feedback reads stay public, since the hub uses them). `admin.html` shows an admin-password box, keeps the token in that tab's `sessionStorage` only, and re-locks on a 401/503. 12 new backend tests (99 total). U6/U7/U9 will build on this same dependency.
- [x] U9 **(built 2026-09-26; live after the redeploy; decided 2026-09-26)**: An optional email field on accounts, used only so the user can verify a password-reset request BY HAND (no automatic emails, no email-sending service). Shown to the admin (which is exactly why the admin page must be secured to the user and the AIs only); never shown to other players and never in any public stat. A signed-in player can add, change or remove it from the hub account panel. Needs a matching update to `terms.html`'s privacy wording. **Built:** `users.email` (optional, lower-cased, validated, max 254), `GET`/`PUT /users/me/email` (sign-in required; null or empty removes it), shown only in the admin Accounts panel, never in any public endpoint (tested), an email block in the hub account panel (add/change/remove) and updated `terms.html` privacy wording. 15 new backend tests (122 total).
- [x] U10 **(done 2026-09-26; decided 2026-09-26)**: SOL: replace "click Fund Research ~20 times" with a real research tree. **Shape:** ~20 nodes between levels that split and rejoin (Civ-6 style); the first tree rejoins at Near Bodies, then splits again and rejoins at Far Bodies. **Costs:** varied per node, but each level's nodes add up to the same total as today (Near Bodies 1000, Far Bodies 5000). **Effects:** most nodes give a small real bonus (e.g. "Space-Grade Mining Equipment" = +10% Iron per click, "Efficient Smelting" = cheaper miners), themed nodes like "Space Travel". **Saves:** existing research progress converts to nodes already researched so nobody loses progress. Currently `RESEARCH_TIERS` is two flat progress bars filled 50 Iron at a time. **Done:** see `games/sol/CLAUDE.md`'s Research tree section.
...
- [x] U14 **(done 2026-09-26 as measure-then-fix; decided 2026-09-26)**: Split each game into separately-loaded "screens" (research tree, achievements, changelog, settings, tutorial…). **Plan (approved):** measure first — per-game boot time and initial DOM size — then split the heaviest 2-3 (likely Continuum, Le Champ de Mots, SOL) into on-demand screens, sharing the mechanism with U4's opening screens. **Result (2026-09-26):** measured every game in the browser pane. Boot to playable is 1.1-2.1s warm (the cold first visit is the shared Pyodide CDN download, which per-game splitting cannot shrink) and the initial DOM is 280-1532 nodes. A first pass over 20 calls suggested Continuum `render()` 18ms and Le Champ de Mots `render()` 34ms, but a steady-state re-measure (after a warm-up call) showed those were inflated by first-call warm-up: Continuum steady `render()` is 2.8ms. The one real hotspot found by profiling each `render()`'s sub-calls was Le Champ de Mots' `render_farm()`, which rewrote five DOM properties on every plot cell every render; it now remembers each cell's and row's last-written values and only writes what changed (steady render about 3.3ms, 4 new tests, 668 game tests green). Every other game is 1.5-5ms per call, and with Continuum's new tick clock its `render()` only runs when a season passes or you act. **Conclusion:** no game needs splitting into on-demand screens for speed today; U4's opening screens already give the structural benefit. If lag is ever felt, note the game and moment and it can be profiled the same way (time each sub-render after one warm-up call).
