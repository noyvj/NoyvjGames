...
- [x] L22 **(checked off but moved — needs your input to proceed)**: Let the cultural-notes toggle show a small "new note available" indicator when a freshly-unlocked row has cultural context worth reading. L4a removed row-unlock pacing (every row is open from the start), so a "freshly-unlocked row" no longer happens — this needs redefining before it can be built. Moved to `planning/FOR-YOU.md` as Q3.
...
- [x] R2-K26 **(checked off but moved — needs your input to proceed)**: Continuum: you said "rather than closing it, let's keep making more eras." Build an eighth era beyond Space Age (sim content, research tier, sustainability mechanic, log/transition beats, info-panel content, Three.js scene, tests), then keep going. K26b's scoping note (`games/continuum/CLAUDE.md`) is written; its 5 open questions are in `planning/FOR-YOU.md` as Q2. Nothing left for me to do here until you answer them — the actual era build starts once you do.
