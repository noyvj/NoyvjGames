...
- [ ] Z23: A lightweight shared mechanism for a game to show different flavor text/art on real-world dates (an optional seasonal Easter-egg layer) — **you specifically want this event-based**, e.g. a week-long "holiday" event (a Christmas-themed Canopy tree-planting push earning a "Christmas 2026" profile badge). Design the shared mechanism with that kind of event in mind, not just a palette swap. Opt-in per game, not a mandatory rollout — no per-game checklist needed until a specific game opts in. Seeded as its own doc (2026-09-22): concrete per-game event ideas (real-world date window, flavor change, badge reward) for SOL + all 8 climate-quartet games + Le Champ de Mots, plus a design sketch for the shared mechanism itself (a per-game `EVENTS` config list piggybacking on the existing achievements/`achievements_earned` plumbing — zero new backend). Still not built — real per-game checklist items get added here once you answer that doc's picks. Folded into `planning/IMPROVEMENT-IDEAS-ROUND-3.md`'s section N (2026-09-22) so it answers alongside everything else, then the standalone doc (`planning/IMPROVEMENT-IDEAS-SEASONAL-EVENTS.md`) was deleted (2026-09-22) once confirmed fully absorbed there — section N is now the only copy.
...
- [ ] R2-K26: Continuum: you said "rather than closing it, let's keep making more eras." Build an eighth era beyond Space Age (sim content, research tier, sustainability mechanic, log/transition beats, info-panel content, Three.js scene, tests), then keep going. *(Recovered: the request was reduced to a scoping note, K26b — now written, see `games/continuum/CLAUDE.md`'s "K26b: Era 8 scoping" section. This line stays open until the actual eighth-era build lands; K26b's own 5 open questions need your answers first.)*
