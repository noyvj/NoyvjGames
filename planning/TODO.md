...
- [ ] U14 **(decided 2026-09-26, ready to build; user hasn't profiled lag)**: Split each game into separately-loaded "screens" (research tree, achievements, changelog, settings, tutorial…). **Plan (approved):** measure first — per-game boot time and initial DOM size — then split the heaviest 2-3 (likely Continuum, Le Champ de Mots, SOL) into on-demand screens, sharing the mechanism with U4's opening screens. **Measured 2026-09-26 (step 1 done, Chrome pane, warm cache, localhost):** boot to a playable game is only 1.1-2.1s for every game (Pyodide itself is cached; the cold first-ever visit is dominated by the Pyodide CDN download, which no per-game splitting can shrink); initial DOM is small everywhere (Canopy/Aftermath/Herd/Thaw/Loop/Drift 280-330 nodes, Grid/Tide about 420, Continuum 509, Trade Empire 730, SOL 1251, Le Champ de Mots 1532). Steady-state cost per call: Loop 1.7ms, Thaw 1.5ms, SOL tick 2.5ms (per 100ms tick), Canopy tick 3.5ms, Trade Empire tick 5.2ms, **Continuum render() 18ms, Le Champ de Mots render() 34ms**, both over a 16ms frame, so a click in those two can visibly hitch. **Conclusion:** startup is not the problem, per-render work in Continuum and Le Champ de Mots is. **Plan for the rest of U14 (still open):** (a) Le Champ de Mots: render only the active screen (farm, review, practice, progress) and refresh a screen when it is opened; (b) Continuum: skip updating hidden panels (City Views, Council Minutes, Founder's Log, achievements, changelog, benchmark) unless open, and rebuild the research list only when its inputs change; (c) re-measure after each and stop when both are under ~8ms.
