...
**Game:** Trade Empire (working title)
**Did:** Moved the pre-build plan out of `games/trade/trade-empire-plan.md` into `games/trade-empire/CLAUDE.md` (same pattern SOL and the climate games followed) and built Milestone 1: a founding-contract intro, one ship, and one fully manual route between two fixed colonies (Aurum Station, which mines Ore, and Verdant Reach, which grows Grain — each needs what the other makes). The loop is Load Cargo → Depart → a short multi-tick transit → auto-sell on arrival, with total profit ticking up as a running total. No automation, need-tracking, or market fluctuation yet — those are explicitly later milestones (3, 4, 6), not gaps in this one. Test harness (fake DOM/Pyodide shims) copied from Canopy's pattern rather than rebuilt from scratch.
**Result:** Milestone 1 playable and verified live in-browser (loaded cargo, departed, watched the transit countdown, confirmed the arrival sale and profit update). 13 new tests passing. Not yet linked from the hub — hub integration is a later milestone in every game here, and this one's no different.
